...
import gestalt.render.plugin.Viewport;
...
public class UsingAPersonalPointOfView
    ...
    private Plane _myDisplay;

    private Cube _myPersonalViewEntity;
    ...
    public void setup() {
        ...
        BilateralBin mySharedBin = new BilateralBin(100);
        ...
        _myDisplay = drawablefactory().plane();
        _myDisplay.material().color.set(1, 0.5f, 0.5f, 0.75f);
        _myDisplay.material().addPlugin(_myPersonalView.texture());
        _myDisplay.setPlaneSizeToTextureSize();
        _myDisplay.origin(SHAPE_ORIGIN_TOP_LEFT);
        _myDisplay.position().set(displaycapabilities().width / -2,
                                  displaycapabilities().height / 2);
        bin(BIN_2D_FOREGROUND).add(_myDisplay);

        /* create a shape to reveal the position of the personal point of view */
        _myPersonalViewEntity = drawablefactory().cube();
