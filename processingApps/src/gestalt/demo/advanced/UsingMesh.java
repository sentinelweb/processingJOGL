...
import gestalt.impl.jogl.shape.JoglVBOMesh;
import gestalt.impl.jogl.shape.JoglVBOMesh.VBOModifier;
...
public class UsingMesh
    ...
    public void setup() {
        ...
        /* add modifier */
        if (_myMesh instanceof JoglVBOMesh && USE_VBO_MODIFIER) {
            ( (JoglVBOMesh) _myMesh).addModifier(new MyVBOModifier());
        }

        /* create texture */
        TexturePlugin myTexture = drawablefactory().texture();
        myTexture.load(bitmapfactory().getBitmap(Resource.getStream("demo/common/auto.png"), "auto"));
    ...
    public void loop(float theDeltaTime) {
        ...
        if (!event().mouseDown) {
             ...
             */
            if (_myMesh instanceof JoglVBOMesh) {
                if (!USE_VBO_MODIFIER) {
                    /* modify vertex data */
                    for (int i = 0; i < 200; i++) {
                        myVertexCounter += 3;
