...
import gestalt.candidates.materialplugin.ByteBitmap3D;
import gestalt.candidates.materialplugin.JoglMaterialPluginTexture3D;
import gestalt.render.AnimatorRenderer;
import gestalt.shape.Mesh;
import gestalt.texture.MovieProducer;
import gestalt.texture.bitmap.ByteBitmap;

import data.Resource;

import gestalt.impl.jogl.shape.JoglPlane;
...
public class Using3DTexture
    ...
    public void setup() {
        ...
        JoglPlane myPlane = new JoglPlane();
        myPlane.material().addTexture().load(bitmapfactory().getBitmap(Resource.getStream("demo/common/auto.png")));
        bin(BIN_3D).add(myPlane);

        /* create texture and store its ID. */
        _myImageTexture = new JoglMaterialPluginTexture3D();

        /* load movie */
        MovieProducer myMovieProducer = bitmapfactory().getMovieProducer(Resource.getPath("demo/common/subway.mov"));
        ...
        for (int i = 0; i < myDepth; i++) {
            ByteBitmap myBitmap = myMovieProducer.getBitmap();
