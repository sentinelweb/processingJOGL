...
class SimplexNoiseFilter extends GLTextureFilter
{
    public SimplexNoiseFilter(PApplet parent, String filename)
    {
        ...
        noiseParams = new SimplexNoiseParameters(parent);
        super.initFilter(filename, noiseParams);
    ...
    {
        noiseParams.parScale = 1.0f / (w * h);
        noiseParams.parTime = time;
        srcTexArray[3] = srcTex;        
        super.apply(srcTexArray, destTex, noiseParams);
    }
    ...
    {    
        ...
        permTex.putIntBuffer(buffer, GLConstants.TEX4);
    }
    ...
    protected void createSimplexTex()
    {
        ...
        simplexTexParams.target = GLTexture.ONEDIM;        
        ...
        simplexTex.putIntBuffer(buffer, GLConstants.TEX4);        
    }    
...
// There is no real need to create this SimplexNoiseParameters class,
// since GLTextureFilterParameters already defines two float parameters.
// Just to give an example on how to derive texture parameter classes.
class SimplexNoiseParameters extends GLTextureFilterParameters
{
    public SimplexNoiseParameters(PApplet parent)
    {
        super(parent);
        
        parTime = parScale = 0.0f;
    }
    ...
    public void getParamIDs(GLSLShader shader)
    {
        ...
        timeUniform = shader.getUniformLocation("time");
    ...
    public void setParamValues()
    {
        ...
        if (-1 < timeUniform) gl.glUniform1fARB(timeUniform, parTime);      
        if (-1 < scaleUniform) gl.glUniform1fARB(scaleUniform, parScale);       
    }
    
    public float parTime;
    public float parScale;
